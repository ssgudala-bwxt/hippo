#!/usr/bin/env bash
# Wrapper for test/tests/timesteppers/foam_tstep_insert — replicates the
# TestHarness pipeline (tests file: foam_controlled_tstep_insert/
# {setup,run,verify} — note: the [Tests] block name in this test's `tests`
# file is a copy-paste leftover from foam_controlled_tstep_insert).
