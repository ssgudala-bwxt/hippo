#!/usr/bin/env bash
# Wrapper for test/tests/variables/foam_variable — mirrors tests file behavior.
# Usage: bash run_test.sh [all|setup|run|verify|run_parallel|reconstruct|verify_parallel]

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HIPPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
PYTHONPATH="${HIPPO_ROOT}/python:${PYTHONPATH:-}"
export PYTHONPATH

cd "${SCRIPT_DIR}"

step() { echo; echo "=== $* ==="; }

# foamCleanCase is not available in all OpenFOAM installs (e.g. minimal/site
# builds without bin/tools/CleanFunctions). Reimplement the equivalent
# cleanup manually: drop generated time directories (keep the initial "0"),
# decomposed processor dirs, and other run artifacts.
clean_case() {
  local dir="$1"
  rm -rf "${dir}"/constant/polyMesh "${dir}"/processor* "${dir}"/postProcessing \
         "${dir}"/VTK "${dir}"/dynamicCode "${dir}"/probes "${dir}"/*.foam "${dir}"/log.*
  local d
  for d in "${dir}"/[0-9]*; do
    [ -e "$d" ] || continue
    [ "$(basename "$d")" = "0" ] && continue
    rm -rf "$d"
  done
}

PASS=0
FAIL=0

run_setup() {
  step "setup: clean_case + blockMesh + decomposePar"
  clean_case foam
  blockMesh -case foam
  decomposePar -case foam -force
}

run_run() {
  step "run: hippo-opt (serial)"
  if srun --mpi=pmi2 -K -n 1 hippo-opt -i main.i; then
    echo "PASS (run)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (run): hippo-opt exited non-zero"
    FAIL=$((FAIL + 1))
  fi
}

run_verify() {
  step "verify: transfer_test / test_variable_transfer + test_wall_heat_flux_transfer"
  if python3 -m pytest test.py -v; then
    echo "PASS (verify)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (verify)"
    FAIL=$((FAIL + 1))
  fi
}

run_run_parallel() {
  step "run_parallel: hippo-opt (n=4)"
  if srun --mpi=pmi2 -K -n 4 hippo-opt -i main.i; then
    echo "PASS (run_parallel)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (run_parallel): hippo-opt exited non-zero"
    FAIL=$((FAIL + 1))
  fi
}

run_reconstruct() {
  step "reconstruct: reconstructPar"
  if reconstructPar -case foam; then
    echo "PASS (reconstruct)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (reconstruct)"
    FAIL=$((FAIL + 1))
  fi
}

run_verify_parallel() {
  step "verify_parallel: transfer_test / test_variable_transfer + test_wall_heat_flux_transfer"
  if python3 -m pytest test.py -v; then
    echo "PASS (verify_parallel)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (verify_parallel)"
    FAIL=$((FAIL + 1))
  fi
}

summary() {
  echo
  echo "=== Summary: ${PASS} passed, ${FAIL} failed ==="
  [ "${FAIL}" -eq 0 ]
}

run_all() {
  run_setup
  run_run
  run_verify
  run_setup
  run_run_parallel
  run_reconstruct
  run_verify_parallel
  summary
}

MODE="${1:-all}"
case "$MODE" in
  all) run_all ;;
  setup) run_setup ;;
  run) run_run; summary ;;
  verify) run_verify; summary ;;
  run_parallel) run_run_parallel; summary ;;
  reconstruct) run_reconstruct; summary ;;
  verify_parallel) run_verify_parallel; summary ;;
  *)
    echo "Usage: $0 [all|setup|run|verify|run_parallel|reconstruct|verify_parallel]"
    exit 1
    ;;
esac
RESULT=$?

echo
if [ "${RESULT}" -eq 0 ]; then
  echo "=== ALL STEPS PASSED ==="
else
  echo "=== FAILURES DETECTED ==="
  exit 1
fi
