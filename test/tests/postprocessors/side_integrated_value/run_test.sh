#!/usr/bin/env bash
# Wrapper for test/tests/postprocessors/side_integrated_value — replicates the
# TestHarness pipeline (tests file: postprocessor_test/{setup,run,
# verify_serial,run_parallel,verify_parallel,boundary_err,invalid_foam_scalar,
# check_valid_func_obj}).
# Usage: bash run_test.sh [all|setup|run|verify_serial|run_parallel|verify_parallel|boundary_err|invalid_foam_scalar|check_valid_func_obj]
...
run_boundary_err() {
  run_expect_err "boundary_err" "Boundary 'top1' not found in FoamMesh." \
    Postprocessors/t_avg/boundary=top1
}

run_invalid_foam_scalar() {
  run_expect_err "invalid_foam_scalar" "No Foam scalar or vector called 'T1'." \
    Postprocessors/t_avg/foam_variable=T1
}

run_check_valid_func_obj() {
  run_expect_err "check_valid_func_obj" 'Invalid option "wallHeatFlux1" in MooseEnum' \
    Postprocessors/heat_flux/function_object='wallHeatFlux1'
}
...
case "$MODE" in
  ...
  boundary_err) run_boundary_err; summary ;;
  invalid_foam_scalar) run_invalid_foam_scalar; summary ;;
  check_valid_func_obj) run_check_valid_func_obj; summary ;;
  *)
    echo "Usage: $0 [all|setup|run|verify_serial|run_parallel|verify_parallel|boundary_err|invalid_foam_scalar|check_valid_func_obj]"
    exit 1
    ;;
esac
RESULT=$?
