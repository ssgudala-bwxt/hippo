#!/usr/bin/env bash
# Wrapper for test/tests/bcs/receiver_pp — replicates the TestHarness pipeline.
# Usage: bash run_test.sh [all|setup|diffusivity_coeff_err|run|verify|run_diffusivity_cv|verify_diffusivity_cv]
...
run_diffusivity_coeff_err() {
  run_expect_err "diffusivity_coeff_err" "Diffusivity 'kappa1' is neither a Foam volScalarField nor a scalar" \
...
case "$MODE" in
  all) run_all ;;
  setup) run_setup ;;
  diffusivity_coeff_err) run_diffusivity_coeff_err; summary ;;
  run) run_run ;;
  verify) run_verify ;;
  run_diffusivity_cv) run_diffusivity_cv ;;
  verify_diffusivity_cv) run_verify_diffusivity_cv ;;
  *)
    echo "Usage: $0 [all|setup|diffusivity_coeff_err|run|verify|run_diffusivity_cv|verify_diffusivity_cv]"
    exit 1
    ;;
esac
RESULT=$?
