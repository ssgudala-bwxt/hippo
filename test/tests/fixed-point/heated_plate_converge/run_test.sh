#!/usr/bin/env bash
# Wrapper for test/tests/fixed-point/heated_plate_converge — replicates the
# TestHarness pipeline (tests file: flow_over_heated_plate_regression/
# {setup,run(exodiff),prep_verify,verify,clean}).
...
run_run() {
  step "run: hippo-opt (n=4, Picard/fixed-point regression) + exodiff"
  ...
  if exodiff -f "${SCRIPT_DIR}/exodiff_filter.txt" heated_plate_out.e gold/heated_plate_out.e; then
    echo "PASS (exodiff)"
    PASS=$((PASS + 1))
  else
    echo "FAILED (exodiff)"
...
run_prep_verify() {
  step "prep_verify: reconstructPar + unpack gold regression tarball"
  reconstructPar -case fluid-openfoam
  tar xvf gold/heated_plate_regression.tar.gz -C gold/
}

run_verify() {
  step "verify: compare fluid-openfoam/ against gold/ reference"
