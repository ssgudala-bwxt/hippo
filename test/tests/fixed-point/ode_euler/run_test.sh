#!/usr/bin/env bash
# Wrapper for test/tests/fixed-point/ode_euler — replicates the
# TestHarness pipeline (tests file: create_reference/{setup,run,copy},
# ode_euler/{setup,run,verify,prep_clean}).
...
run_run() {
  step "ode_euler setup + run (2 fixed-point iterations, default)"
  clean_case foam
  blockMesh -case foam
