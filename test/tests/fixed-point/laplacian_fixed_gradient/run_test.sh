#!/usr/bin/env bash
# Wrapper for test/tests/fixed-point/laplacian_fixed_gradient — replicates the
# TestHarness pipeline (tests file: create_reference/{setup,run,copy},
# laplacian_fixed_gradient/{setup,run,verify,prep_clean}).
...
run_run() {
  step "laplacian_fixed_gradient setup + run (2 fixed-point iterations, default)"
  clean_case foam
  blockMesh -case foam
