#!/usr/bin/env bash
# Wrapper for test/tests/fixed-point/ode_crank_nicolson — replicates the
# TestHarness pipeline (tests file: create_reference/{setup,run,copy},
# ode_crank_nicolson/{setup,run,verify,prep_clean}).
...
run_run() {
  step "ode_crank_nicolson setup + run (2 fixed-point iterations, default)"
  clean_case foam
  blockMesh -case foam
